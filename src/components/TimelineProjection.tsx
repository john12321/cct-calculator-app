...
export const TimelineProjection: FC<TimelineProjectionProps> = ({
  ...
  const isOverPlanned =
    !isOpenEnded && accrual.totalWteMonthsCompleted >= adjustedLength - 0.009;
  ...
  if (isOpenEnded && last) {
    ...
    regimeNote = `The timeline already covers the required training, so the completion date is the last recorded end date (${formatDate(last.endDate)}).`;
    ...
    regimeNote = `The timeline does not yet cover the required training. The completion date extrapolates from ${formatDate(fromDate)} at ${wteLabel} — the most recent grade WTE in the timeline. Add a project-forward period to change the rate (e.g. switch to a higher WTE).`;
  } else {
    regimeNote = "";
  }
